...
describe('GET /login', () => {
	...
	it('400s with invalid_return_url when return_url is missing', async () => {
		const res = await exports.default.fetch('https://auth.flyindycenter.com/login', {
			redirect: 'manual'
		});
	...
	it('rejects a loopback return_url in prod mode (COOKIE_DOMAIN=.flyindycenter.com)', async () => {
		const res = await exports.default.fetch(
			'https://auth.flyindycenter.com/login?return_url=' +
