...
import { fetchVatsimProfile } from '../../src/auth/vatsim';
...
describe('fetchVatsimProfile', () => {
	...
	it('throws IdentityError(vatsim_profile_failed, 502) on non-200', async () => {
	...
	it('throws IdentityError(vatsim_profile_failed, 502) on malformed 200 response', async () => {
		const mockFetch = vi.fn().mockResolvedValue(
